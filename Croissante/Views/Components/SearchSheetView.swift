...
struct SearchSheetView: View {
    ...
    private var fullScreenSearchView: some View {
        GeometryReader { _ in
            ZStack {
                fullScreenBackground.ignoresSafeArea()
            }
            ...
            .background(
                ZStack {
                    Capsule(style: .continuous)
                        .fill(fullScreenControlMaterial)
                    Capsule(style: .continuous)
                        .fill(fullScreenControlFill)
                    Capsule(style: .continuous)
                        .stroke(fullScreenControlBorder, lineWidth: 1)
                    Capsule(style: .continuous)
                        .stroke(fullScreenControlInnerBorder, lineWidth: 0.8)
                        .padding(1)
                }
            )
    ...
    private var searchTextField: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                .fill(isDark ? AppColors.nocturneSurface.opacity(0.70) : Color.black.opacity(0.04))
        )
    }
    ...
    private var pasteFromClipboardButton: some View {
        Button(action: pasteFromClipboard) {
            Image(systemName: "doc.on.clipboard")
                ...
                .background(
                    Circle()
                        .fill(isDark ? AppColors.nocturneSurfaceElevated.opacity(0.88) : Color.black.opacity(0.06))
                )
        }
    }
