...
private struct SettingsTabGearAnimationBridge: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: ProbeView, context: Context) {
        context.coordinator.update(from: uiView, spinToken: spinToken)
    }
    ...
    final class Coordinator: NSObject {
        ...
        func refresh(from view: UIView) {
            update(from: view, spinToken: currentSpinToken)
        }

        func update(from view: UIView, spinToken: Int) {
...
private struct DiscoverScreen: View {
    ...
    private func handleGalaxyPinch(_ event: ExplorePinchEvent, containerFrame: CGRect) {
        ...
        case .changed:
            ...
            let targetCount = 28
            var filled = previewWords
            while filled.count < targetCount {
                filled += previewWords
            }

            galaxyPinchLatched = true
            galaxyWords = Array(filled.prefix(targetCount))
...
private struct WordGalaxyOverlay: View {
    ...
    @State private var isDraggingOrbit = false
            ...
            .simultaneousGesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        ...
                        orbitDragOffset += Double(delta) / 200.0
                    }
                    .onEnded { _ in
...
private struct GalaxyWordCard: View {
    ...
    var body: some View {
        ...
        .frame(width: 82, height: 112, alignment: .topLeading)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .stroke(colorScheme == .dark ? Color.white.opacity(0.30) : Color.black.opacity(0.08), lineWidth: colorScheme == .dark ? 1 : 0.8)
        )
        .shadow(color: Color.black.opacity(colorScheme == .dark ? 0.24 : 0.10), radius: colorScheme == .dark ? 10 : 14, x: 0, y: colorScheme == .dark ? 5 : 8)
    }
}
...
private struct ProgressWordRow: View {
    ...
    var body: some View {
        ...
        .contentShape(Rectangle())
        .simultaneousGesture(
            DragGesture(minimumDistance: 0, coordinateSpace: .named(progressRevealCoordinateSpace))
                .onEnded { value in
