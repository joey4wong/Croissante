...
private struct DiscoverScreen: View {
    ...
    private func scheduleWidgetDataWrite(delayNanoseconds: UInt64 = 250_000_000) {
        ...
        pendingWidgetWriteTask = Task { @MainActor in
            ...
            guard !Task.isCancelled, appState.hasCompletedInitialResourceLoad else { return }
            writeWidgetData()
        }
    }
...
private struct WordGalaxyOverlay: View {
    ...
    private func projectedCards(
        ...
        for (index, word) in words.enumerated() {
            ...
            let zOrder = isInPile ? 1800 + Double(count - slot) : -p.wz
...
private struct ActiveDiscoverCardHost: View {
    ...
    private func renderedCard(progress: Double, transitionRequest: GalaxySelectedCardTransitionRequest?) -> some View {
        ...
        .id(word.id)
        .frame(width: containerSize.width, height: containerSize.height)
        .position(x: containerSize.width / 2, y: containerSize.height / 2 + 12)
        ...
        ZStack {
            ForEach((0..<backLayerCount).reversed(), id: \.self) { layer in
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .stroke(galaxyAccentBorder(isDarkMode: isDarkMode).opacity(accentOpacity(layer)), lineWidth: 1.0)
                    )
